...
public class CassandraController {
	...
	public ResponseEntity<SimpleTable> fetchRecordFromSimpleTable(@PathVariable("id") String id, @PathVariable("name") String name) {
		Optional<SimpleTable> fetchData = cassandraRepository.findByIdAndName(id, name);
		if(!fetchData.isPresent()) {}
		return new ResponseEntity<SimpleTable>(fetchData.get(), HttpStatus.OK);
	}
